...
Web developers are increasingly using icon fonts in place of images, since they're lightweight vectors that can easily resize as needed. While Materialize includes a set of icons (which is what was used for the "hamburger" button on the mobile menu), it doesn't include many other icons that might be useful, including those for social sites. We're going to add in the [Font Awesome](http://fontawesome.io/get-started/) fonts. 

To access them, you need to get a (free) code from Font Awesome allowing you to access the online version of their icon font. You'll put this line into the head of the document, in the icons section, above the materialize icons. It will look like this, but with **your personal code replacing the abcde12345.js reference**:
